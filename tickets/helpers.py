...
def _send_reassigned_email(ticket, new_assigner):
    """Send email to the student and the new assigner when a ticket is reassigned."""
    if not settings.EMAIL_HOST_USER or not settings.EMAIL_HOST_PASSWORD:
        return
    if not ticket.student.email or not new_assigner.email:
        return
    ...
    # Email to student
    subject_student = "Your Support Ticket Has Been Reassigned - {subject}".format(
        subject=ticket.subject
    )
    body_student = (
        "Dear {first_name},\n\n"
        "Your support ticket has been reassigned to {assigner_name}.\n\n"
        "Ticket Subject: {ticket_subject}\n\n"
        "You may review the ticket and provide any further clarification using the link below:\n"
        "{ticket_url}\n\n"
        "Kind regards,\n"
        "Clarify Support Team"
    ).format(
        first_name=ticket.student.first_name or "student",
        assigner_name=new_assigner.get_full_name(),
        ticket_subject=ticket.subject,
        ticket_url=ticket_url,
    )
    ...
    # Email to new assigner
    subject_assigner = "A Support Ticket Has Been Assigned to You - {subject}".format(
        subject=ticket.subject
    )
    body_assigner = (
        "Dear {assigner_name},\n\n"
        "You have been assigned a new support ticket by another staff member.\n\n"
        "Ticket Subject: {ticket_subject}\n"
        "Student: {student_name}\n\n"
        "You can find this ticket in your “Claimed” tickets.\n\n"
        "or view it using the link below:\n"
        "{ticket_url}\n\n"
        "Kind regards,\n"
        "Clarify Support Team"
    ).format(
    ...
    send_mail(
        subject=subject_assigner,
        message=body_assigner,
        from_email=settings.DEFAULT_FROM_EMAIL or settings.EMAIL_HOST_USER,
        recipient_list=[new_assigner.email],
        fail_silently=False,
    )
